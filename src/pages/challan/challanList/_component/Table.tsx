...
import ChallanColumn from './ChallanColumn'
...
const DataTable: React.FC<DataTableProps> = ({
  ...
  const { data,isLoading } = useGetChallanListQuery({
    search: text,
    page: paginationModel.page + 1,
  })
  const { data: urlData, isLoading: loading } = useDownloadChallanQuery(
    {
      id: selectedRow?.id,
    },
    { skip: !selectedRow },
  ...
  const handleChallanPdfReport = async () => {
    const name = 'Challan Report'
    // await fileDownloader(urlData, name)
  }
  const columns = ChallanColumn(theme, setSelectedRow, handleChallanPdfReport,loading)
