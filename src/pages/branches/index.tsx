/* eslint-disable @typescript-eslint/no-explicit-any */
import AddIcon from '@mui/icons-material/Add'
import {
  Box,
  Button,
  Grid,
  InputAdornment,
  styled,
  TextField,
  Theme,
  Typography,
  useMediaQuery,
  useTheme,
} from '@mui/material'
import { GridSearchIcon } from '@mui/x-data-grid'
// import { setText } from '../../../redux/feature/search/searchSlice'
// import { useAppDispatch, useAppSelector } from '../../../redux/hook'
// import useDebounce from '../../../utils/useDebounce'import { SetStateAction, useEffect, useState } from 'react'

// import {
//   useCreateLocationMutation,
//   useUpdateLocationMutation,
// } from '../../../redux/feature/product/ProductApi'

import DataTable from './_component/Table'
// import { CREATE_ACTION } from './constant'
// import LocationModal from '../../../components/modals/LocationModal'
// import './style.css'
import { SetStateAction, useEffect, useState } from 'react'
import BranchModal from '../../components/modals/BranchModal'
import {
  useCreateBranchMutation,
  useUpdateBranchMutation,
} from '../../redux/feature/brances/branchesApi'
import { setSelectBranchesAction } from '../../redux/feature/brances/branchesSlice'
import { setText } from '../../redux/feature/search/searchSlice'
import { useAppDispatch, useAppSelector } from '../../redux/hook'
import useDebounce from '../../utils/useDebounce'
import { BRANCH_CREATE_ACTION } from './constant'
// import { setLocationAction } from '../../../redux/feature/product/ProductSlice'
// import { LOCATION_CREATE_ACTION } from './constant'

export default function Branches() {
  const [isOpen, setIsOpen] = useState<boolean>(false)
  const theme: Theme = useTheme()
  const isSmallScreen = useMediaQuery(theme.breakpoints.down('sm'))
  const [createBranch] = useCreateBranchMutation()
  const [updateBranch] = useUpdateBranchMutation()
  const [selectedRow, setSelectedRow] = useState<any>(null)

  const [searchTerm, setSearchTerm] = useState<string>('')
  const debouncedSearchTerm = useDebounce(searchTerm, 500) // Use the custom hook
  const dispatch = useAppDispatch()
  const { text } = useAppSelector(state => state.search)
  useEffect(() => {
    dispatch(setText(debouncedSearchTerm))
  }, [debouncedSearchTerm])
  // Handle search input change
  const handleSearchChange = (event: {
    target: { value: SetStateAction<string> }
  }) => {
    setSearchTerm(event.target.value)
  }

  const ProductButton = styled(Button)({
    textTransform: 'none',
    fontSize: 14,
    padding: '6px 12px',

    lineHeight: 1.5,
    backgroundColor:
      theme.palette.mode === 'dark'
        ? theme.palette.primary.main
        : theme.palette.secondary.main,
    '&:hover': {
      backgroundColor:
        theme.palette.mode === 'dark'
          ? theme.palette.primary.main
          : theme.palette.secondary.main,
    },
    color: 'white',
    margin: isSmallScreen ? '5px 0' : '10px 0',
  })
  return (
    <Box
      className="lotList"
      sx={{
        background:
          theme?.palette?.mode === 'dark'
            ? '#202020'
            : 'rgba(255, 255, 255, 1)',
        padding: '33px 24px',
        borderRadius: '8px',
      }}
    >
      <Typography variant="subtitle1" style={{ fontWeight: 500 }}>
        Branch List
      </Typography>
      <Box
        sx={{
          display: 'flex',
          flexDirection: isSmallScreen ? 'column' : 'row',
          justifyContent: 'space-between',
        }}
      >
        {isSmallScreen ? (
          <Grid
            container
            spacing={2}
            sx={{ marginBottom: isSmallScreen ? '5px 0' : '16px 0' }}
          >
            <Grid item xs={12} md={6} lg={3}>
              <TextField
                fullWidth
                placeholder="Search"
                value={searchTerm} // Bind search term state to input
                onChange={handleSearchChange} // Update search term state on input change
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
                      <GridSearchIcon />
                    </InputAdornment>
                  ),
                }}
                sx={{
                  padding: '2px 0',
                  background:
                    theme.palette.mode === 'dark'
                      ? 'rgba(57, 57, 57, 1)'
                      : 'rgba(239, 243, 244, 1)',
                  borderColor: 'transparent',
                  borderRadius: '8px',
                }}
              />
            </Grid>
          </Grid>
        ) : (
          <div></div>
        )}
        <ProductButton
          startIcon={<AddIcon />}
          onClick={() => {
            dispatch(setSelectBranchesAction(BRANCH_CREATE_ACTION))
            setIsOpen(true)
          }}
          // sx={{
          //   backgroundColor:
          //     theme.palette.mode === 'dark'
          //       ? theme.palette.primary.main
          //       : theme.palette.secondary.main,
          //   '&:hover': {
          //     backgroundColor:
          //       theme.palette.mode === 'dark'
          //         ? theme.palette.primary.main
          //         : theme.palette.secondary.main,
          //   },
          //   color: 'white',
          //   margin: isSmallScreen ? '5px 0' : '10px 0',
          // "&.css-t54gau-MuiButtonBase-root-MuiButton-root":{
          //   textTransform:"capitalize"
          // }
          // }}
        >
          Create
        </ProductButton>
      </Box>
      <DataTable
        text={text}
        setIsOpen={setIsOpen}
        setSelectedRow={setSelectedRow}
        selectedRow={selectedRow}
      />
      {isOpen && (
        <BranchModal
          updateBranch={updateBranch}
          createBranch={createBranch}
          isOpen={isOpen}
          setIsOpen={setIsOpen}
          selectedRow={selectedRow}
        />
      )}
    </Box>
  )
}
